...
class netsender {
	...
	static netsender* createSender(netsender::PROTOCOL_TYPE protocol, netsender::NETSENDER_TYPE type, std::string connectServer, int port);
...
//	virtual int send_buf(std::string str) = 0;
	virtual int send_buf(std::string str,SOCKETINFO* socketinfo = nullptr) = 0;
//	virtual int send_buf(const char* data, int len) = 0;
	virtual int send_buf(const char* data, int len,SOCKETINFO* socketinfo = nullptr) = 0;
